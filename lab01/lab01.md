...
1. Использует ли ваш браузер HTTP версии 1.0 или 1.1? Какая версия HTTP работает на
   ...
   - Мой браузер использует версию 1.1 (так как запрос с версией 1.1)
   - На сервере работает версия 1.1 (так как ответ тоже с версией 1.1)
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - Может принимать английсикй
   ![image](2.png)
   - Ещё информация о версии браузера, ОС, и некоторых параметрах компьютера
   ![image](3.png)
3. Какой IP-адрес вашего компьютера? Какой адрес сервера gaia.cs.umass.edu?
   - `Source Address: 192.168.1.120` - адрес компьютера
   - `Destination Address: 128.119.245.12` - адрес сервера
...
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - `If-Modified-Since: Mon, 19 Feb 2024 06:59:02 GMT` Отображается время последней модификации файла на серере
4. Какой код состояния HTTP и фраза возвращаются сервером в ответ на этот второй запрос
   ...
   - Явно ничего не возвращается
   - Возвращается код: `Status Code: 304`: `[Status Code Description: Not Modified]`
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - ![image](8.png)
   - 3 GET запроса на 2 адреса
   - ip.dst == 128.119.245.12, ip.dst == 178.79.137.164
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Вроде бы последовательно, второй запрос был отправлен после получения ответа на первый, хотя раз быстрее отправить запрос, чем ждать пока все пройдет по сети, если бы они были параллельные, то сначала было бы два запроса, а потом два ответа
...
2. Когда ваш браузер отправляет сообщение HTTP GET во второй раз, какое новое поле включается в сообщение HTTP GET?
   - `Authorization: Basic d2lyZXNoYXJrLXN0dWRlbnRzOm5ldHdvcms=\r\n` и `Credentials: wireshark-students:network`
